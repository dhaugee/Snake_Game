...
def invert(img_matrix):
    ...
    for i in range(len(img_matrix)):
        for j in range(len(img_matrix[i])):
            for m in range(len(img_matrix[i][j])):
                img_matrix[i][j][m] = 255 - img_matrix[i][j][m]
    return img_matrix
...
def grayscale(img_matrix):
    ...
    for i in range(len(img_matrix)):
        for j in range(len(img_matrix[i])):
            total = img_matrix[i][j][0] + img_matrix[i][j][1] + img_matrix[i][j][2]
            grayscale = int(total/3)
            for m in range(len(img_matrix[i][j])):
                img_matrix[i][j][m] = grayscale
    return img_matrix
...
def mirror(img_matrix):
    ...
    for i in range(len(img_matrix)):
        colors += 1
    colors = colors//2
    firsthalf = img_matrix[:colors]
    otherhalf = firsthalf[:]
    otherhalf.reverse()
    mirrored = firsthalf + otherhalf
    if colors % 2 != 0:
        mirrored.insert(len(firsthalf), img_matrix[colors])
        return mirrored
    return mirrored
...
def right_diff(img_matrix):
    ...
    '''
    for i in range(len(img_matrix)):
        for j in range(len(img_matrix[i])-1):
            for m in range(len(img_matrix[i][j])):
                newnum = ((img_matrix[i][j][m])-(img_matrix[i][j+1][m]))*8
                if newnum > 255:
                    newnum = 255
                elif newnum < 0:
                    newnum = 0
                img_matrix[i][j].pop(m)
                img_matrix[i][j].insert(m, newnum)
        img_matrix[i].pop(len(img_matrix[i])-1)
        img_matrix[i].append([0,0,0])
    return img_matrix
